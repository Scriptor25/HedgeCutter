...
std::ostream& hcut::lang::operator<<(std::ostream& out, const Type& type)
{
	return out << type.Name;
}

hcut::lang::TypePtr hcut::lang::Parser::NextType(const std::string& name)
{
	return NextType(Type::Create(name));
}
