...
namespace hcut::lang
{
	...
	struct DefStmt : Stmt
	{
		static DefStmtPtr Create(TypePtr type, const std::string& name) { return new DefStmt(Symbol::Create(type, name)); }
	...
	struct SymStmt : Stmt
	{
		static SymStmtPtr Create(TypePtr result, const std::string& name, const std::vector<SymbolPtr>& args, bool vararg, StmtPtr supplier) { return new SymStmt(Symbol::Create(result, name), args, vararg, supplier); }
	...
	struct IfStmt : Stmt
	{
		static IfStmtPtr Create(ExprPtr condition, StmtPtr ontrue, StmtPtr onfalse) { return new IfStmt(condition, ontrue, onfalse); }
