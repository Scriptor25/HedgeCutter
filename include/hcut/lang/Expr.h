...
namespace hcut::lang
{
	...
	struct BinExpr : Expr
	{
		static BinExprPtr Create(const std::string& op, ExprPtr left, ExprPtr right) { return new BinExpr(op, left, right); }
	...
	{
		static CallExprPtr Create(ExprPtr callee, const std::vector<ExprPtr>& args) { return new CallExpr(callee, args); }
	...
	struct IntExpr : Expr
	{
		static IntExprPtr Create(const std::string& value, int radix) { return new IntExpr(std::stoull(value, nullptr, radix)); }
	...
	struct SelExpr : Expr
	{
		static SelExprPtr Create(ExprPtr condition, ExprPtr ontrue, ExprPtr onfalse) { return new SelExpr(condition, ontrue, onfalse); }
